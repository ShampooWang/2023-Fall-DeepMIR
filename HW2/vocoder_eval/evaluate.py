...
def evaluate(y_path_list, y_g_hat_path_list):
    ...
    with torch.no_grad():
        for y_path, y_g_hat_path in tqdm(zip(y_path_list, y_g_hat_path_list), total=len(y_path_list)):
            if _M_STFT or _F0:
                y = load_wav(y_path)
    ...
    if _M_STFT:
        RETURN['M-STFT'] = mrstft_tot / len(y_path_list)        

    if _F0:
        RETURN['log2f0_mean'] = f0_mean_tot / len(y_path_list)
