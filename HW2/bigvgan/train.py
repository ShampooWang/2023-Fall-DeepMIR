...
def train(rank, a, h):
    ...
    wandblogger = wandb.init(
        ...
        name=a.checkpoint_path.split("/")[-1],
        config=a
    )
    ...
    if os.path.isdir(a.checkpoint_path):
        ...
        cp_g = "/work/jgtf0322/Homework/2023-Fall-DLMAG/HW2/exp/ckpt/bigvgan_22khz_80band/g_base"
        cp_do = "/work/jgtf0322/Homework/2023-Fall-DLMAG/HW2/exp/ckpt/bigvgan_22khz_80band/do_base"

        # cp_g = "/work/jgtf0322/Homework/2023-Fall-DLMAG/HW2/exp/ckpt/bigvgan_base_22khz_80band/g_base"
        # cp_do = "/work/jgtf0322/Homework/2023-Fall-DLMAG/HW2/exp/ckpt/bigvgan_base_22khz_80band/do_base"
    ...
    trainset = MelDataset(training_filelist, h, h.segment_size, h.n_fft, h.num_mels,
                          ...
                          fine_tuning=a.fine_tuning, base_mels_path="/work/jgtf0322/Homework/2023-Fall-DLMAG/HW2/dataset/mel/train", is_seen=True)
    ...
    if rank == 0:
        validset = MelDataset(validation_filelist, h, h.segment_size, h.n_fft, h.num_mels,
                              ...
                              base_mels_path="/work/jgtf0322/Homework/2023-Fall-DLMAG/HW2/dataset/mel/small_valid", is_seen=True)
    ...
    def validate(rank, a, h, loader):
        ...
        with torch.no_grad():
            ...
            for j, batch in enumerate(tqdm(loader)):
                ...
                # del batch, x, y, y_mel, y_g_hat, y_g_hat_mel
            ...
            ## Required metrics ##
            ######################
            RETURN = evaluate(input_path_list, save_path_list)
            for key in RETURN:
                print(f"Validation {key}: {RETURN[key]}")
                sw.add_scalar(f"validation/{key}", RETURN[key], steps)
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(tqdm(train_loader)):
            ...
            if rank == 0:
                ...
                if steps % a.summary_interval == 0:
                    ...
                    wandblogger.log(
                        {
                            "train_gen_loss_total": loss_gen_all, 
                            "train_mel_spec_error": mel_error,
                            "train_fm_loss_mpd": loss_fm_f.item(),
                            "train_gen_loss_mpd": loss_gen_f.item(),
                            "train_disc_loss_mpd": loss_disc_f.item(),
                            "train_fm_loss_mrd": loss_fm_s.item(), 
                            "train_gen_loss_mrd": loss_gen_s.item(),
                            "train_disc_loss_mrd": loss_disc_s.item(),
                            "train_grad_norm_mrd":  grad_norm_mrd, 
                            "train_grad_norm_g": grad_norm_g,
        ...
        # checkpointing
        if epoch % a.checkpoint_interval == 0 and steps != 0:
            checkpoint_path = os.path.join(a.checkpoint_path, f"g_ep{epoch}")
            save_checkpoint(checkpoint_path,
                            {'generator': (generator.module if h.num_gpus > 1 else generator).state_dict()})
            checkpoint_path = os.path.join(a.checkpoint_path, f"d_ep{epoch}")
            save_checkpoint(checkpoint_path, 
        ...
        # validation
        if epoch % a.validation_interval == 0 and not a.debug and steps != 0:
            validate(rank, a, h, validation_loader)
