...
def load_wav(full_path, sr_target):
    return librosa.load(full_path, sr=sr_target)
...
class MelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.fine_tuning:
            ...
            song_name, song_id = splitted_file_name[-2], splitted_file_name[-1].split(".")[0]
            mel = np.load(os.path.join(self.base_mels_path, song_name, f"{song_id}.npy"))
