...
def main():
    midi_l = [ str(f) for f in Path("/work/jgtf0322/Homework/2023-Fall-DeepMIR/HW3/dataset/midi_analyzed").glob('**/*.mid') ]
    x_len = 1024 # set the input length. must be same with the model config
    event2word, word2event = pickle.load(open("/work/jgtf0322/Homework/2023-Fall-DeepMIR/HW3/TA_demo/basic_event_dictionary.pkl", 'rb'))


    with Pool(32) as pool:
        all_events = list(tqdm(pool.imap(extract_events, midi_l), total=len(midi_l)))
        ...
        for event in events:
            ...
            if e in event2word:
                ...
                if event.name == 'Note Velocity':
                    ...
                    words.append(event2word['Note_Velocity_21'])
                else:
                    # something is wrong
    ...
    for i, words in enumerate(all_words):
        np.save(f"/work/jgtf0322/Homework/2023-Fall-DeepMIR/HW3/dataset/basic_remi/{i}.npy", words)
        pairs = []
