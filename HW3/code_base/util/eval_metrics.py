...
sys.path.append("/work/jgtf0322/Homework/2023-Fall-DeepMIR/tmp-tools/MusDr")
...
event2word, word2event = pickle.load(open("/work/jgtf0322/Homework/2023-Fall-DeepMIR/HW3/dataset/basic_event_dictionary.pkl", 'rb'))
...
if __name__ == "__main__":
  ...
  test_pieces = [ str(f) for f in Path(opt.output_file_path).glob('**/*.mid') ]
  ...
  print(f"avg. H1: {sum(result_dict['H1']) / len(result_dict['H1'])}")
  print(f"avg. H4: {sum(result_dict['H4']) / len(result_dict['H4'])}")
  print(f"avg. GS: {sum(result_dict['GS']) / len(result_dict['GS'])}")
